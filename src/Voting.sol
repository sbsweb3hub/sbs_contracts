...
contract Voting {
    ...
    mapping(uint32 => mapping(uint8 => mapping(address => bool)))
        public isVoting; // голосовал уже или нет

    function viewProjectResultVoting(
        uint32 _projectId,
        uint8 _step
    ...
    function vote(uint32 _projectId) public {
        ...
        require(
            ...
            "Voting is over or not start yet!"
        );
        require(
            !isVoting[_projectId][stepIsLive][msg.sender],
        ...
        if (
            projectStepsResultVoting[_projectId][stepIsLive] >
            (tokenSupply / 100) * negativeShare
        ) {
