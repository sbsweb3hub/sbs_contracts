...
contract Voting {
    ...
    mapping(uint32 => mapping(uint8 => uint256)) public projectStepsResultVoting; // результаты прохождения этапов, сколько голосов против, начиная с 2: (т.к. 0: паблик, 1: 1-я работа)
    mapping(uint32 => mapping(uint8 => mapping(address => bool))) public isVoting; // голосовал уже или нет

    function viewProjectResultVoting(uint32 _projectId, uint8 _step) public view returns (uint256) {
        return projectStepsResultVoting[_projectId][_step];
    }

    function vote(uint32 _projectId) public {
        uint256 userOrderedTokens = ordering_contract.getUserOrderedTokens(_projectId, msg.sender);
        require(userOrderedTokens != 0, "You haven't the tokens to vote!");
        (uint256 tokenSupply,,,,, bool isProjectAlive,,) = createProject_contract.projectsViewPrice(_projectId);
        ...
        uint256 userVotingPower = userOrderedTokens;
