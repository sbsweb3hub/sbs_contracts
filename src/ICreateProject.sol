...
interface ICreateProject {
    ...
    function projectsViewSteps(uint32 projectId) external view returns 
       (
        uint8, uint[] memory, uint[] memory, uint[] memory, bool[] memory, uint, bool
        );
    
    function projectsViewPrice(uint32 projectId) external view returns (uint, uint, uint, uint, uint, bool, bool, uint);

    function projectsViewMain(uint32 projectId) external view returns (ProjectToken, address, string memory, string memory);
