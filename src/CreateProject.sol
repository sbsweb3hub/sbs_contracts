    ...
    ProjectManagerSteps
{
    ...
    function create(
        ...
        require(
            _amountSteps >= 4 && _timeSteps.length >= 4,
            "Need three or more steps!"
        );
        require(
            _publicSale <= _maxTokenSupply,
            "Incorrect amount public-MaxSupply"
        );
        require(_minTokenSale <= _publicSale, "Incorrect amount min-public");

        uint32 idMain = createProjectMain(_projectName, _projectSymbol);
        uint32 idPrice = createProjectPrice(
            _maxTokenSupply,
            _minTokenSale,
            _price,
            _publicSale
    ...
    function setNewOwnerOfProject(
        uint32 _projectId,
        address _newOwnerOfProject
    ) public onlyOwner {
    ...
    function witchStepAlive(
        uint32 _projectId
    ) public view returns (uint8 step) {
        CreateSteps memory project = getProjectSteps[_projectId];
    ...
    function setApproveClaimAddresses(
        uint32 _projectId,
        address _claimableAddr,
        address _getFundForProjectAddr
    ) private {
        (ProjectToken projectContract, address owner, , ) = projectsViewMain(
            _projectId
        );
        require(msg.sender == owner, "Not an owner!");
        projectContract.setClaimableAddrs(
            _claimableAddr,
            _getFundForProjectAddr
        );
    }
    ...
    function deleteProjectFromArr(uint32 _projectId) public {
        (, address ownerProject, , ) = projectsViewMain(_projectId);
        require(msg.sender == ownerProject, "Not an owner of Project!");
        (, , , , , bool isProjectAlive, , ) = projectsViewPrice(_projectId);
