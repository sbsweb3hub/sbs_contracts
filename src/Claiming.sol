...
contract Claiming {
    ...
    function claimTokens(uint32 _projectId) public {
        ...
        if (reward != 0) {
            ...
            ordering_contract.setTokensAlreadyClaimed(
                _projectId,
                msg.sender,
                reward
            );
    ...
    function earned(
        ...
        uint userOrderedTokens = ordering_contract.getUserOrderedTokens(
            _projectId,
            _account
        );
        require(userOrderedTokens != 0, "You don't have any tokens.");
        (, , , , , bool isProjectAlive, , ) = createProject_contract
        ...
        // нужно это обдумать
        require(
            stepIsLive > 1 && block.timestamp >= dateSteps[1],
            "Still early"
        ); // паблик удачный и врямя завершения паблика
        uint tokensAlreadyClaimed = ordering_contract.getTokensAlreadyClaimed(
            _projectId,
            _account
        );
        ...
        if (isProjectAlive) {
            ...
            for (uint i = 2; i <= amountSteps; i++) {
                ...
                if (isStepAlive) {
                    rewardTime =
                        dateSteps[i - 1] +
                        (dateSteps[i] - dateSteps[i - 1]) /
                        2;
                    if (
                        i == 2 &&
                        block.timestamp > rewardTime &&
                        block.timestamp < dateSteps[i]
                    ) {
                        return
                            (userOrderedTokens /
                                (amountSteps - 1) /
                                rewardTimePerStep[i - 1]) * // сколько токенов в милисекунду
                            (block.timestamp - rewardTime) -
                        ...
                        return
                            (((userOrderedTokens / (amountSteps - 1)) *
                                (i - 2)) +
                                ((userOrderedTokens /
                                    (amountSteps - 1) /
                                    rewardTimePerStep[i - 1]) * // сколько токенов в милисекунду
