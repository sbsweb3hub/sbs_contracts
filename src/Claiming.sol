...
contract Claiming {
    ...
    function claimTokens(uint32 _projectId) public {
        uint256 reward = earned(_projectId, msg.sender);
        if (reward != 0) {
            (ProjectToken projectContract,,,) = createProject_contract.projectsViewMain(_projectId);

            (uint256 amountSteps,,, uint256[] memory dateSteps,,,) =
    ...
    function earned(uint32 _projectId, address _account) public view returns (uint256) {
        uint256 userOrderedTokens = ordering_contract.getUserOrderedTokens(_projectId, _account);
        require(userOrderedTokens != 0, "You don't have any tokens.");
        (,,,,, bool isProjectAlive,,) = createProject_contract.projectsViewPrice(_projectId);
        ...
        uint256 tokensAlreadyClaimed = ordering_contract.getTokensAlreadyClaimed(_projectId, _account);
        ...
        if (isProjectAlive) {
            bool isStepAlive;
            uint256 rewardTime;
            for (uint256 i = 2; i <= amountSteps; i++) {
                ...
                if (isStepAlive) {
                    ...
                    if (i == 2 && block.timestamp > rewardTime && block.timestamp < dateSteps[i]) {
                        return (userOrderedTokens / (amountSteps - 1) / rewardTimePerStep[i - 1]) // сколько токенов в милисекунду
                            * (block.timestamp - rewardTime) - tokensAlreadyClaimed;
                    } else if (i == 2 && block.timestamp < rewardTime) {
                        return 0;
                    } else if (i > 2 && block.timestamp < rewardTime) {
                        return (userOrderedTokens / (amountSteps - 1)) // tokensPerStep
                            * (i - 2) - tokensAlreadyClaimed;
                    } else if (i > 2 && block.timestamp > rewardTime && block.timestamp < dateSteps[i]) {
                        return (
                            ((userOrderedTokens / (amountSteps - 1)) * (i - 2))
                                + (
