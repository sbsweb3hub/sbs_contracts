...
contract Ordering {
    ...
    function changeBlastPointsOperator(address _blastPointsAddress, address _newOperator) external {
        ...
        IBlastPoints(_blastPointsAddress).configurePointsOperatorOnBehalf(address(this), _newOperator);
    }
    ...
    event OrderedTokens(address buyer, uint256 projectId, uint256 amount);
    event RefundUser(address user, uint256 amountForRefund);
    event SBSFund(uint32 _projectId, address _onwerProject, uint256 _sbsFundForProject);
    event SBSFeeReward(uint32 _projectId, address owner, uint256 _sbsFeeReward);

    mapping(uint256 => mapping(address => uint256)) public recievedMoneyFromInvestor; // деньги, которые покупатель зарезервировал под определенного проект
    mapping(uint256 => mapping(address => uint256)) public orderedTokens; // оплаченные токены по адресам покупателей
    mapping(uint256 => mapping(address => uint256)) public tokensAlreadyClaimed; //количество токенов переданных инвестору
    ...
    function order(uint32 _projectId) public payable {
        ...
        (,,, uint256[] memory dateSteps, bool[] memory isStepsPerProject,,) =
            createProject_contract.projectsViewSteps(_projectId);
        require(block.timestamp <= dateSteps[1], "Too late to invest!");

        (uint256 tokenSupply,, uint256 minTokenSale, uint256 price, uint256 publicSale,,, uint256 fundsForProject) =
            createProject_contract.projectsViewPrice(_projectId);

        uint256 _amount = msg.value * 10 ** 18 / price;
        uint256 _maxPerTx = (publicSale / 100) * 10; // 10% от максимального сейла, чтобы дать многим шанс купить
        ...
        createProject_contract.orderPrice(_projectId, tokenSupply, fundsForProject);
    ...
    function refundUsers(uint32 _projectId) public {
        ...
        (uint256 tokenSupply,, uint256 minTokenSale,,, bool isProjectAlive,,) =
            createProject_contract.projectsViewPrice(_projectId);
        ...
        uint256 amountForRefund = recievedMoneyFromInvestor[_projectId][msg.sender];
    ...
    function getRecievedMoneyFromInvestor(uint32 _projectId) public view returns (uint256 invested, uint256 all) {
