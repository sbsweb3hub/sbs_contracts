...
contract CreateProjectTest is Test {
    ...
    uint256 _maxTokenSupply = 1000;
    uint256 _minTokenSale = 100;
    uint256 _price = 1 ether;
    uint256 _publicSale = 200;
    ...
    function setUp() public {
        ...
        // Fill timeSteps
        _timeSteps = new uint256[](_amountSteps);
        for (uint256 i = 0; i < _amountSteps; i++) {
    ...
    function test_MoneyFromInvestor() public {
        ...
        ordering.order{value: 1 ether}(id);
        (uint256 invested1, uint256 all1) = ordering.getRecievedMoneyFromInvestor(id);
        ...
        ordering.order{value: 1 ether}(id);
        (uint256 invested2, uint256 all2) = ordering.getRecievedMoneyFromInvestor(id);
