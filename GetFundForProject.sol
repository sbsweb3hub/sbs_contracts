  ...
  // поочередное получение проектом всех траншей 
    function getNextFund(uint32 _projectId) public reentrancyGuard {
        ...
        uint32 votingTime = 300; // 604800;
            if(stepIsLive == 1) {
              require((block.timestamp < dateSteps[1] && tokenSupply == publicSale)
              ||
              (block.timestamp > dateSteps[stepIsLive]), "Sale is still ongoing"); // если паблик закончился раньше времени - можно забрать первый транш
            } else {
